...
pub fn select_input() -> (cpal::Device, cpal::SupportedStreamConfig) {
    ...
    let devices: Vec<cpal::Device> = host.input_devices().expect("failed to enumerate input devices").collect();
    ...
    let config = device.default_input_config().expect("device has no default input config");
